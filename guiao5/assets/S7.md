...
As scripts [Client.py](/trabalho-pratico/scripts/Client.py) e [Server.py](/trabalho-pratico/scripts/Server.py) constituem uma implementação elementar
...
Onde _g_ e _p_ são os paâmetros públicos do protocolo; _(x, g<sup>x</sup>)_ e _(y, g<sup>y</sup>)_ são os pares de chaves de Alice e Bob respectivamente; e `Kmaster` é o segredo estabelecido pelo protocolo.

### PROG: `Client_dh.py` e `Server_dh.py`

Pretende-se adaptar os programas realizados no ponto anterior para que a chave por eles utilizada resulte da execução do protocolo de acordo de chaves _Diffie-Hellman_. Para isso iremos fazer uso da funcionalidade oferecida pela biblioteca `cryptography`, utilizando a classe [`dh`](https://cryptography.io/en/latest/hazmat/primitives/asymmetric/dh/), que acabará por "esconder" a matemática apresentada atrás.
